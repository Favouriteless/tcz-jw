...
public class BulletHoleParticle extends TextureSheetParticle {
    ...
    public BulletHoleParticle(ClientLevel world, double x, double y, double z, Direction direction, BlockPos pos, String ammoId, String gunId) {
            ...
            if (minecraft.player != null) {
                ...
                // Obtener las escalas del jugador
                playerHeightScale = ScaleUtils.getModelHeightScale(minecraft.player); // Método para obtener la escala de altura
                playerWidthScale = ScaleUtils.getModelWidthScale(minecraft.player);   // Método para obtener la escala de anchura

                // Calcular la escala promedio del jugador para usar en el tamaño de la partícula
                float playerScale = (playerHeightScale + playerWidthScale) / 2.0F;

                // Definir los valores mínimos y máximos para la escala y el tamaño de la partícula
                float minScale = 0.001F;
                float maxScale = 1.0F;
                float minSize = 0.001F;
                float maxSize = 0.05F;

                // Normalizar la escala del jugador entre 0 y 1
                float normalizedScale = Mth.clamp((playerScale - minScale) / (maxScale - minScale), 0.0F, 1.0F);

                // Mapear la escala normalizada al tamaño de la partícula
                this.quadSize = minSize + (maxSize - minSize) * normalizedScale;
            } else {
                this.quadSize = 0.01F; // Tamaño por defecto
            }

          //  world.playLocalSound(x, y, z, SoundEvents.ANVIL_LAND, SoundSource.BLOCKS, 0.9F, 1.0F, true);
